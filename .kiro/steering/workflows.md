  ...
  execute: async ({ inputData, mastra }) => {
    ...
    try {
      ...
      if (retrieveResult.toolResults && retrieveResult.toolResults.length > 0) {
        const toolResult = retrieveResult.toolResults.find(tr => tr.toolName === 'vectorQueryTool') || retrieveResult.toolResults[0];
      ...
      if (contexts.length > 0) {
        try {
          ...
          console.log(`[${requestId}] ⚠️ Reranking failed, using original order`);
        }
      }
