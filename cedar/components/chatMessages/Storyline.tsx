...
import type { StorylineMessage, StorylineSection } from 'cedar-os';
import { AnimatePresence, motion, easeOut, backOut } from 'framer-motion';
...
const Storyline: React.FC<StorylineProps> = ({ message }) => {
	...
	const [edgePositions, setEdgePositions] = useState<
		Array<{ fromY: number; toY: number; centerX: number }>
	>([]);
    ...
    return;
		...
		message.sections.forEach((_, index) => {
			if (index === 0) {
     return;
   } // Skip the first one as it's already visible
   ...
   case "function": { throw new Error('Not implemented yet: "function" case') }
   default:
				return null;
		}
	};
