...
export default function AuthPanel({ onAuth }: AuthPanelProps) {
    const [selectedRole, setSelectedRole] = useState<
        (typeof DEMO_ROLES)[0] | null
    >(null)
    const [loading, setLoading] = useState(false)
    const [authProgress, setAuthProgress] = useState(0)

    const generateJWT = useCallback(
        async (role: (typeof DEMO_ROLES)[0]) => {
            setLoading(true)
            setAuthProgress(0)
            ...
            try {
                const secret: Uint8Array = new TextEncoder().encode(
                    process.env.NEXT_PUBLIC_JWT_SECRET ?? 'dev-secret'
                )
                        ...
                        <div className="flex items-center justify-center space-x-4 mb-6 group hover-lift">
                            ...
                            <div className="flex flex-col">
                                ...
                                <CardDescription className="text-bold-serif text-muted-foreground text-lg">
                                    Secure role-based access to enterprise
                                    knowledge
                                </CardDescription>
                            </div>
                        </div>
                    ...
                    <CardContent className="relative">
                        ...
                        <Alert className="mb-6 border-2 border-accent/30 bg-accent/5">
                            ...
                            <AlertDescription className="text-accent/80">
                                This demo showcases role-based access control.
                                Select a role to see how information access is
                                governed based on your permissions.
                            </AlertDescription>
                        </Alert>
                        ...
                        <Tabs defaultValue="roles" className="w-full">
                            <TabsList className="grid w-full grid-cols-2 mb-6">
                                ...
                                <TabsTrigger value="info" className="font-bold">
                                    Access Levels
                                </TabsTrigger>
                            </TabsList>
