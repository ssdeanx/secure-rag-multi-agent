    ...
    <Card className="h-full flex flex-col hover-lift hover-glow transition-colors">
      <CardHeader className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          {meta.readingTime !== undefined && meta.readingTime !== '' ? (
        ...
        <CardTitle className="text-xl leading-snug">
          <Link href={`/blog/${meta.slug}`} className="focus:outline-none focus:ring-2 focus:ring-primary/60 rounded-sm">
            {meta.title}
          </Link>
        </CardTitle>
        ...
        {meta.author !== undefined && meta.author !== '' ? (
          <p className="text-xs text-muted-foreground pt-1">By {meta.author}</p>
        ) : null}
      </CardHeader>
    </Card>
  );
