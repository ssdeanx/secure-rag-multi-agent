...
interface ContactPayload { name: string; email: string; category: string; message: string; }

export function ContactForm() {
  const [form, setForm] = useState<ContactPayload>({ name: '', email: '', category: 'general', message: '' });
    ...
    >
      ...
      <div aria-live="polite" aria-atomic="true" className="min-h-[1.5rem]">
        {error !== null && error.length > 0 ? (
          <p id="contact-error" role="alert" className="text-sm text-destructive">{error}</p>
        ) : success ? (
