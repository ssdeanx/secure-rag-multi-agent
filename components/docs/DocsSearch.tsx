...
interface DocIndexEntry { slug: string; title: string; headings: Array<{ id: string; text: string; level: number }> }
...
export function DocsSearch() {
  ...
  useEffect(() => {
    ...
    fetch('/api/docs-index')
      .then(r => r.json())
  .then((data: DocIndexEntry[]) => { if (!cancelled) { setAllDocs(data); } })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    const scored = allDocs.map(d => {
      ...
      const headingScore = Math.max(0, ...d.headings.map(h => fuseScore(query, h.text)));
      return { doc: d, score: Math.max(titleScore, headingScore) };
    }).filter(s => s.score > 0).sort((a,b) => b.score - a.score).slice(0, 10).map(s => s.doc);
    setResults(scored);
  }, [query, allDocs]);
        ...
        <Command className="rounded-md border bg-background">
          <CommandList>
            ...
            <CommandGroup heading="Results">
              {results.map(r => (
                <CommandItem key={r.slug} onSelect={() => onSelect(`/docs/${r.slug}`)} value={r.slug} className="cursor-pointer">
