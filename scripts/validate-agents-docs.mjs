...
const REQUIRED_SECTIONS = ['## Persona', '## Change Log']
...
function parseMeta(content, file) {
    ...
    try {
        ...
        if (missing.length) {
            return {
                ok: false,
                errors: [`${file}: Missing meta fields: ${missing.join(', ')}`],
            }
        }
        if (!Array.isArray(json.tags) || json.tags.length === 0) {
            return {
                ok: false,
                errors: [`${file}: tags must be a non-empty array`],
            }
        }
        ...
        if (!/^\d{4}-\d{2}-\d{2}T/.test(json.last_updated)) {
            return {
                ok: false,
                errors: [`${file}: last_updated should be ISO-8601`],
            }
        }
        ...
        return {
            ok: false,
            errors: [`${file}: Invalid JSON in AGENTS-META (${e.message})`],
        }
    }
}
...
function validateSections(content, file) {
    ...
    return missing.length
        ? [`${file}: Missing required section(s): ${missing.join(', ')}`]
        : []
}
