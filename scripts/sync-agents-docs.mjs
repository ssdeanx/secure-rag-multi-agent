...
function gitChangedAgents(baseRef) {
    const diff = execSync(`git diff --name-only ${baseRef} -- '**/AGENTS.md'`)
        .toString()
        .trim()
    if (!diff) {
        return []
...
function updateFile(path) {
    ...
    if (metaLineIdx === -1) {
        return { path, skipped: true, reason: 'no_meta' }
    }
    const meta = parseMeta(lines[metaLineIdx])
    if (!meta) {
        return { path, skipped: true, reason: 'bad_meta' }
    }
    ...
    function insertChangeLog(note) {
        ...
        const headerIdx = lines.findIndex(
            (l, idx) => idx > clIndex && /\|\s*Version\s*\|/.test(l)
        )
        if (headerIdx === -1) {
            return
        }
    ...
    if (UPDATE_INDEX && /\/AGENTS.md$/.test(path) && meta.applies_to === '/') {
        const indexStart = lines.findIndex((l) =>
            /^## Documentation File Index/.test(l)
        )
        if (indexStart !== -1) {
            const tableHeader = lines.findIndex(
                (l, idx) => idx > indexStart && /^\| Title \| Path/.test(l)
            )
            if (tableHeader !== -1) {
                ...
                const rows = all
                    .map((p) => {
                        ...
                        return `| ${m.title || rel} | [${rel}](${rel}) | ${(m.tags || [])[0]?.replace('layer:', '') || 'n/a'} | ${
                            (m.tags || [])
                                .filter((t) => t.startsWith('domain:'))
                                .map((d) => d.replace('domain:', ''))
                                .join(',') || 'n/a'
                        } | ${m.status || 'n/a'} |`
                    })
                    .sort()
    ...
    if (!changed) {
        return { path, skipped: true, reason: 'no_change_needed' }
    }
...
function main() {
    ...
    if (!changed.length) {
        if (!QUIET) {
            console.log('No AGENTS.md changes detected against', BASE)
        }
        process.exit(0)
    }
