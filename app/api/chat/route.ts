...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.log(
                `Verified user: ${rawPayload.sub ?? 'unknown'} with roles ${roles.join(', ')} in tenant ${tenant}`
            )
        } catch (verifyError) {
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    controller.enqueue(
                        encoder.encode(
                            `data: ${JSON.stringify({ content: 'Processing your request...\n\n' })}\n\n`
                        )
                    )
                    ...
                    const workflow =
                        mastra.getWorkflows()['governed-rag-answer']
