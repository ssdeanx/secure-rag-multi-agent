...
// Define type for indexed document result to replace 'any'
    ...
    try {
        ...
        const documents = corpusFiles
            ...
            .map((file) => {
                ...
                let classification: 'public' | 'internal' | 'confidential' =
                    'public'
                let allowedRoles: string[] = ['employee']

                if (
                    fileName.includes('confidential') ||
                    fileName.includes('hr')
                ) {
        ...
        if (result.status === 'success') {
            log.info(
                `Indexing completed: ${result.result.indexed} indexed, ${result.result.failed} failed`
            )
