...
export function useRoadmapState(
    ...
    useRegisterState({
        ...
        key: 'nodes',
        description:
            'Product roadmap features and bugs that can be managed through conversation',
        stateSetters: {
            addNode: {
                name: 'addNode',
                ...
                execute: (currentNodes, setValue, args: unknown) => {
                    const nodeArg = (
                        args as { node?: Node<FeatureNodeData> } | undefined
                    )?.node
                    if (!nodeArg) {
                        return
                    }
            ...
            changeNode: {
                ...
                execute: (currentNodes, setValue, args: unknown) => {
                    const updatedNode = (
                        args as { newNode?: Node<FeatureNodeData> } | undefined
                    )?.newNode
                    if (!updatedNode) {
                        return
                    }
