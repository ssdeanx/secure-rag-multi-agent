...
describe('streamUtils', () => {
  ...
  describe('streamProgressUpdate', () => {
    ...
    it('should handle different status values', () => {
      ...
      statuses.forEach((status) => {
        streamProgressUpdate(mockController, `Status: ${status}`, status);
      });

      expect(mockController.enqueue).toHaveBeenCalledTimes(statuses.length);
    });
  });
  ...
  describe('streamJSONEvent', () => {
    it('should encode JSON event correctly', () => {
      ...
      const testData = { type: 'test', value: 42, nested: { key: 'value' } };
  ...
  describe('handleTextStream', () => {
    it('should handle text stream and return full text', async () => {
      ...
      const result = await handleTextStream(mockStreamResult, mockController);
    ...
    it('should encode each chunk correctly', async () => {
      ...
      const calls = (mockController.enqueue as ReturnType<typeof vi.fn>).mock.calls;
    ...
    it('should handle stream with special characters', async () => {
      const chunks = ['{"test": "value"}', '<html>content</html>', 'emoji 🎉'];
      const mockTextStream = (async function* () {
        for (const chunk of chunks) {
          yield chunk;
        }
  ...
  describe('Integration tests', () => {
    it('should work together in a typical streaming workflow', async () => {
      ...
      const response = createSSEStream(async (controller) => {
        ...
        streamJSONEvent(controller, { type: 'start', timestamp: Date.now() });

        const result = await handleTextStream(mockStreamResult, controller);
