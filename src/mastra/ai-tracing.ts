...
export class LangfuseExporter implements AITracingExporter {
    ...
    constructor(config: LangfuseExporterConfig) {
        ...
        if (!hasPublicKey || !hasSecretKey) {
            this.logger.error(
                'LangfuseExporter: Missing required credentials, exporter will be disabled',
                {
                    hasPublicKey,
                    hasSecretKey,
                }
    ...
    private async handleSpanStarted(span: AnyExportedAISpan): Promise<void> {
        ...
        const langfuseParent = this.getLangfuseParent({
            traceData,
            span,
            method,
        })
        ...
        const langfuseSpan =
            span.type === AISpanType.LLM_GENERATION
                ? langfuseParent.generation(payload)
                : langfuseParent.span(payload)
    ...
    private async handleSpanUpdateOrEnd(
        ...
        if (!langfuseSpan) {
            this.logger.warn(
                'Langfuse exporter: No Langfuse span found for span update/end',
                {
    ...
    private initTrace(span: AnyExportedAISpan): void {
        if (!this.client) {
            this.logger.error(
                'Langfuse exporter: initTrace called without a Langfuse client - exporter is disabled',
                {
                    traceId: span.traceId,
                    spanId: span.id,
                }
    ...
    private buildSpanPayload(
        ...
        if (span.type === AISpanType.LLM_GENERATION) {
            const llmAttr =
                attributes as unknown as Partial<LLMGenerationAttributes>
