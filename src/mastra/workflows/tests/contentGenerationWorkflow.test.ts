...
describe('contentGenerationWorkflow', () => {
  beforeEach(() => {
    ...
    (editorAgent.generateVNext as any).mockResolvedValue({
      ...
      object: {
        editedContent: 'This is the edited and refined content with improvements.',
  ...
  it('should execute workflow with valid input data and return final content', async () => {
    ...
    const stream = run.streamVNext({
      inputData: {
        ...
        requirements: 'Write a beginner-friendly introduction to TypeScript covering basic types and syntax.',
        tone: 'professional',
  ...
  it('should use default tone when not provided', async () => {
    ...
    const stream = run.streamVNext({
      inputData: {
        ...
        requirements: 'Cover query optimization and caching strategies.',
  ...
  it('should generate content for different content types', async () => {
    const contentTypes: Array<'blog' | 'marketing' | 'technical' | 'business'> = ['blog', 'marketing', 'technical', 'business'];
