...
const mastraEventTemplates: Record<MastraEventType, Record<string, unknown>> = {
    start: {
        type: 'start',
        from: 'WORKFLOW',
        payload: { workflowId: 'financialAnalysisWorkflowV5' },
    },
    'step-start': {
        type: 'step-start',
        from: 'WORKFLOW',
        payload: {
            stepName: 'analyzeQuery',
            startedAt: Date.now(),
        },
    },
    'tool-call': {
...
const analyzeQueryStepV5 = createStep({
    ...
    execute: async ({ inputData }) => {
        ...
        try {
            const streamController = inputData.streamController as ReadableStreamDefaultController<Uint8Array> | undefined
            ...
            const watchlistSymbols: string[] = cedarContext?.watchlist?.map(item => typeof item === 'string' ? item : (item as unknown as { symbol: string }).symbol) ?? []
            ...
            const primaryAsset: string = (inputData.symbols[0]) || (watchlistSymbols[0]) || 'UNKNOWN'
            ...
            if (streamController) {
                streamJSONEvent(streamController, {
                    ...mastraEventTemplates['step-result'],
                    queryAnalysis: {
                        intent,
                        complexity,
                        primaryAsset,
...
const orchestrateAnalysisStepV5 = createStep({
    ...
    execute: async ({ inputData }) => {
        ...
        try {
            ...
            if (streamController) {
                streamJSONEvent(streamController as ReadableStreamDefaultController<Uint8Array>, {
                    ...mastraEventTemplates['step-start'],
                    payload: { ...(mastraEventTemplates['step-start'].payload as object), stepName: 'orchestrateAnalysis' }
                })
            }
            ...
            const results = await orchestrateAgentsV5(
                ...
                streamController as ReadableStreamDefaultController<Uint8Array> | undefined
            )
...
const orchestrateAgentsV5 = async (
    ...
    for (const agentName of strategy.requiredAgents) {
        try {
            ...
            if (streamController) {
                streamJSONEvent(streamController, {
                    ...mastraEventTemplates['tool-output'],
                    agent: agentName,
                    progress: 0,
                })
            }
            ...
            if (streamController) {
                streamJSONEvent(streamController, {
                    ...mastraEventTemplates['tool-result'],
                    agent: agentName,
                })
            }
...
const assessQualityStepV5 = createStep({
    ...
    execute: async ({ inputData }) => {
        ...
        try {
            ...
            if (streamController) {
                streamJSONEvent(streamController as ReadableStreamDefaultController<Uint8Array>, mastraEventTemplates['step-start'])
            }
