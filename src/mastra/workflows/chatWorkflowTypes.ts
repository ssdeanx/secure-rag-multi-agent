...
const CedarNodeDataSchema = z.object({
  ...
  status: z.enum(['done', 'planned', 'backlog', 'in progress']).default('planned').describe('Current development status'),
  ...
  comments: z
    ...
    .default([]).describe('User comments'),
});
...
const CedarNodeSchema = z.object({
  ...
  position: z
    ...
    .optional().describe('Node position in UI'),
...
export const CedarLLMResponseSchema = z.object({
  ...
  object: CedarActionResponseSchema.optional().describe('Structured action response'),
});
...
export const CedarChatInputSchema = z.object({
  ...
  }).optional().describe('Cedar OS context and state access'),
});
