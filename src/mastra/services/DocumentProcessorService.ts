...
export class DocumentProcessorService {
    ...
    async processDocument(
        ...
        try {
            console.log(
                ...
                `Starting document processing pipeline for: ${doc.docId}`
            )
            ...
            console.log(
                'DOCUMENT_PROCESSOR_SERVICE',
                `Document chunked: ${chunkingResult.totalChunks} chunks using ${chunkingResult.method} (doc length: ${content.length} chars)`
            )
            console.log(
                `Document chunked: ${chunkingResult.totalChunks} chunks using ${chunkingResult.method} (doc length: ${content.length} chars)`
            )
            ...
            console.log(
                'DOCUMENT_PROCESSOR_SERVICE',
                `Embeddings generated: ${embeddingResult.embeddings.length} embeddings, ${embeddingResult.batchesProcessed} batches`
            )
            console.log(
                `Embeddings generated: ${embeddingResult.embeddings.length} embeddings, ${embeddingResult.batchesProcessed} batches`
            )
            ...
            console.log(
                'DOCUMENT_PROCESSOR_SERVICE',
                `Cleaned up ${deleteResult.deleted} existing vectors for ${doc.docId}`
            )
            ...
            console.log(
                'DOCUMENT_PROCESSOR_SERVICE',
                `Document processing completed for ${doc.docId} in ${processingTime}ms`
            )
            console.log(
                `Document processing completed for ${doc.docId} in ${processingTime}ms`
            )
    ...
    async processDocuments(
        ...
        for (let i = 0; i < docs.length; i++) {
            ...
            if ((i + 1) % 10 === 0 || i === docs.length - 1) {
                ...
                console.log(
                    `Batch progress: ${completed}/${docs.length} processed, ${successful} successful`
                )
            }
        }
    ...
    private async readDocument(doc: DocumentInput): Promise<string> {
        try {
            ...
            if (content.length < 50) {
                console.warn(
                    `Very small document (${content.length} chars): ${doc.docId}`
                )
            }
    ...
    async getProcessingEstimate(
        ...
        console.log(
            'DOCUMENT_PROCESSOR_SERVICE',
            `Analyzing ${docs.length} documents for processing estimates`
        )
        console.log(
            `Analyzing ${docs.length} documents for processing estimates`
        )
