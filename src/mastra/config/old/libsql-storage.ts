...
export const createLibSQLStore = (tracingContext?: {
    context?: any
    runtimeContext?: RuntimeContext
    currentSpan?: {
        createChildSpan(_input: TracingSpanInput): {
            end(options: SpanEndInput): void
        }
    }
}) => {
...
export const createLibSQLVectorStore = (tracingContext?: {
    context?: unknown
    runtimeContext?: RuntimeContext
    currentSpan?: {
        createChildSpan(_input: TracingSpanInput): {
    ...
    const startTime = Date.now()
    const databaseUrl =
        process.env.VECTOR_DATABASE_URL ?? STORAGE_CONFIG.VECTOR_DATABASE_URL
    ...
    const initSpan = tracingContext?.currentSpan?.createChildSpan({
        ...
        input: {
            ...
            hasAuthToken: !!(
                process.env.VECTOR_DATABASE_AUTH_TOKEN ??
...
export const searchSimilarContent = async (
    ...
    try {
        ...
        const embeddings: number[][] = isNumberMatrix(embedResult)
            ? embedResult
            : isNumberMatrix(
                    (embedResult as { embeddings?: unknown }).embeddings
                )
...
export async function createVectorIndex(
    ...
    tracingContext?: {
        currentSpan?: {
            createChildSpan(input: TracingSpanInput): {
                end(options: SpanEndInput): void
            }
        }
    }
...
export async function searchMemoryMessages(
    ...
    try {
        ...
        const recalledMessagesArray: unknown[] = Array.isArray(
            (recalled as { messages?: unknown[] }).messages
        )
            ? (recalled as { messages: unknown[] }).messages
            : Array.isArray((recalled as { messagesV2?: unknown[] }).messagesV2)
              ? (recalled as { messagesV2: unknown[] }).messagesV2
        ...
        const relevantMessages = recalledMessagesArray.map(
            (msg: unknown, idx: number) => {
                const msgObj = msg as {
                    id?: string
                    role?: string
                    sender?: string
                ...
                const safeId =
                    msgObj.id ?? `${threadId}-msg-${idx}-${Date.now()}`
                const role = normalizeRole(
                    msgObj.role ?? msgObj.sender ?? msgObj.roleName
                )
                const content =
                    msgObj.content ??
                    msgObj.parts?.map((p) => p.text ?? p.content).join('') ??
                    ''
                const createdAtRaw = msgObj.createdAt ?? msgObj.timestamp
                const createdAt =
                    typeof createdAtRaw !== 'undefined'
...
export const simplifyMessages = (messages: readonly MessageV2[]) => {
    return messages.map((msg: Readonly<MessageV2>) => ({
        ...
        content:
            msg.content ??
            msg.parts?.map((part) => part.text ?? part.content).join('') ??
            '',
...
export const getOrCreateUserResource = async (
    memory: Memory,
    userId: string
) => {
    try {
...
export const updateUserWorkingMemory = async (
    memory: Memory,
    userId: string,
    updates: {
...
export function extractChunkMetadata(
    ...
    chunks.forEach((chunk, index) => {
        ...
        if (
            extractParams.title !== undefined &&
            extractParams.title !== false
        ) {
            if (typeof extractParams.title === 'boolean') {
                const firstLine = chunk.content.split('\n')[0]?.trim()
                const firstSentence = chunk.content.split(/[.!?]/)[0]?.trim()
                enhancedMetadata.extractedTitle =
                    firstLine || firstSentence || `Chunk ${index + 1}`
            } else {
        ...
        if (
            extractParams.summary !== undefined &&
            ...
            if (typeof extractParams.summary === 'boolean') {
                ...
                if (
                    extractParams.summary.summaries?.includes('self') ??
                    false
                ) {
        ...
        if (
            extractParams.keywords !== undefined &&
            extractParams.keywords !== false
        ) {
            if (typeof extractParams.keywords === 'boolean') {
                const words =
                    chunk.content.toLowerCase().match(/\b\w{4,}\b/g) ?? []
                ...
                extractParams.keywords.keywords !== undefined &&
                extractParams.keywords.keywords > 0 &&
                extractParams.keywords.keywords <= 100
            ) {
                // Limit to 100 keywords for performance and relevance
                enhancedMetadata.extractedKeywords = [
                    `great`,
                    `task`,
                    `hello`,
                    'help',
                    ...
                    'second',
                    'third',
                ].slice(0, extractParams.keywords.keywords)
            }
        }

        if (
            extractParams.questions !== undefined &&
            extractParams.questions !== false
        ) {
            if (typeof extractParams.questions === 'boolean') {
                const questions = chunk.content
                    .split(/[.!?]/)
                    .map((s) => s.trim())
                    .filter((s) => s.endsWith('?'))
                    .slice(0, 3)
...
export const performStorageHealthCheck = async (
    context: 'research' | 'report' = 'research'
) => {
    const results = {
        storage: false,
        vectorStore: false,
    ...
    const relevantIndexes =
        context === 'research'
            ? [
                  STORAGE_CONFIG.VECTOR_INDEXES.RESEARCH_DOCUMENTS,
                  STORAGE_CONFIG.VECTOR_INDEXES.LEARNINGS,
              ]
            : [
                  STORAGE_CONFIG.VECTOR_INDEXES.REPORTS,
                  STORAGE_CONFIG.VECTOR_INDEXES.WEB_CONTENT,
              ]
    ...
    try {
        ...
        results.errors.push(
            `Vector store check failed: ${error instanceof Error ? error.message : 'Unknown error'}`
        )
    }
    ...
    for (const indexName of relevantIndexes) {
        try {
            ...
            await vectorStore.query({
                indexName,
                queryVector: new Array(STORAGE_CONFIG.DEFAULT_DIMENSION).fill(
                    0
                ),
                topK: 1,
            })
            ...
            results.errors.push(
                `Index '${indexName}' check failed: ${error instanceof Error ? error.message : 'Unknown error'}`
            )
        }
    }
    ...
    log.info(
        'For storage documentation, refer to mastraDocs path: reference/storage/',
        {
            context,
        }
    )
...
export const initializeStorageSystem = async (
    ...
    try {
        ...
        if (healthCheck.healthy) {
            ...
            log.warn('Storage system initialized with issues', {
                healthCheck,
                context,
            })
