...
export async function initVectorStore() {
    try {
        ...
        log.info('PgVector index initialized successfully', {
            indexName: 'governed_rag',
            dimension: 1568,
        })
    } catch (error) {
...
export class VectorStoreUtils {
    ...
    private static retryAttempts = parseInt(
        process.env.VECTOR_STORE_RETRY_ATTEMPTS ?? '3'
    )
    private static retryDelay = parseInt(
        process.env.VECTOR_STORE_RETRY_DELAY ?? '1000'
    )
        ...
        for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
            try {
                ...
                if (attempt === this.retryAttempts) {
                    log.error(
                        `Operation ${operationName} failed after ${this.retryAttempts} attempts`,
                        {
                            ...context,
                            attempt,
    ...
    private static validateUpsertInput(
        ...
        if (vectors.length !== metadata.length) {
            throw new Error(
                `Vectors length (${vectors.length}) must match metadata length (${metadata.length})`
            )
        }

        if (ids && ids.length !== vectors.length) {
            throw new Error(
                `IDs length (${ids.length}) must match vectors length (${vectors.length})`
            )
        }
        ...
        for (let i = 1; i < vectors.length; i++) {
            if (vectors[i].length !== dimension) {
                throw new Error(
                    `All vectors must have the same dimension. Expected ${dimension}, got ${vectors[i].length} at index ${i}`
                )
            }
        }
    ...
    static async upsertWithTracing(
        ...
        return this.withRetry(
            async () => {
                const span = this.tracingEnabled
                    ? this.createTracingSpan(AISpanType.GENERIC, {
                          indexName,
                ...
                try {
                    ...
                    throw this.createVectorStoreError(
                        'UPSERT_FAILED',
                        errorMessage,
                        { indexName, vectorCount: vectors.length }
                    )
                }
            },
            'upsert',
                ...
                try {
                    ...
                    const filteredResults =
                        options.scoreThreshold !== null
                            ? result.filter(
                                  (r) => r.score >= options.scoreThreshold!
                              )
                            : result
                    ...
                    log.info('Vector query completed successfully', {
                        ...
                        avgScore:
                            result.length > 0
                                ? result.reduce((sum, r) => sum + r.score, 0) /
                                  result.length
                                : 0,
                    })
     ...
     */
    private static createTracingSpan(
        type: AISpanType,
    ...
    static async healthCheck(): Promise<boolean> {
        try {
            ...
            log.info('PgVector store health check passed', {
