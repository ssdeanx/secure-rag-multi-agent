...
export interface GoogleSheetsToolContext {
    userId?: string
    arcadeUserId?: string
}
...
export const googleSheetsTool = ({
    ...
    if (!arcadeApiKey || arcadeApiKey.trim() === '') {
        throw new Error(
            'ARCADE_API_KEY is required but not provided or is empty'
        )
    }

    if (!arcadeUserId || arcadeUserId.trim() === '') {
        throw new Error(
            'ARCADE_USER_ID is required but not provided or is empty'
        )
    }

    if (!defaultSpreadsheetId || defaultSpreadsheetId.trim() === '') {
        throw new Error(
            'GOOGLE_SHEETS_SPREADSHEET_ID is required but not provided or is empty'
        )
    }
    ...
    return createTool({
        ...
        inputSchema: z
            .object({
                spreadsheet_id: z
                    ...
                    .optional()
                    .describe(
                        'The ID of the Google Spreadsheet, Omitting this would use the default spreadsheet'
                    ),
            })
            .optional()
            .describe(
                'No Input Parameters Needed for this tool, just an empty JSON object would suffice'
            ),
        ...
        execute: async ({ context }) => {
            const finalSpreadsheetId =
                context?.spreadsheet_id ?? defaultSpreadsheetId
            ...
            try {
                ...
                try {
                    ...
                    if (
                        ...
                        log.info(
                            'Tool needs authorization, starting auth flow...'
                        )
                        ...
                        if (auth?.status !== 'completed') {
                            log.info(
                                `Click here to authorize the tool: ${auth?.url}`
                            )
