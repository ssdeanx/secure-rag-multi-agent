...
describe('Web Scraper Tool Tests', () => {
  ...
  describe('HtmlProcessor', () => {
    describe('sanitizeHtml', () => {
      it('should sanitize HTML by removing script and style tags', () => {
        const html = '<html><head><script>alert("xss")</script><style>body{color:red}</style></head><body><h1>Hello</h1></body></html>';
        const result = HtmlProcessor.sanitizeHtml(html);
        expect(result).toBe('<html><head></head><body><h1>Hello</h1></body></html>');
      });
    ...
    describe('extractTextContent', () => {
      ...
      it('should handle HTML parsing errors', () => {
        ...
        expect(() => HtmlProcessor.extractTextContent(html)).toThrow(ScrapingError);
      });
    });
    ...
    describe('htmlToMarkdown', () => {
      ...
      it('should handle conversion errors', () => {
        ...
        const html = '<p>Test</p>';
        expect(() => HtmlProcessor.htmlToMarkdown(html)).toThrow(ScrapingError);
      });
    });
  ...
  describe('ValidationUtils', () => {
    describe('validateUrl', () => {
      it('should validate valid URLs', () => {
        ...
        expect(() => ValidationUtils.validateUrl('http://example.com')).not.toThrow();
      });

      it('should reject invalid URLs', () => {
        expect(() => ValidationUtils.validateUrl('not-a-url')).toThrow(ScrapingError);
        expect(() => ValidationUtils.validateUrl('')).toThrow(ScrapingError);
        expect(() => ValidationUtils.validateUrl('ftp://example.com')).toThrow(ScrapingError);
      });
    });

    describe('sanitizeFileName', () => {
      it('should sanitize filenames by removing invalid characters', () => {
        expect(ValidationUtils.sanitizeFileName('file<name>.txt')).toBe('filename.txt');
        expect(ValidationUtils.sanitizeFileName('file:name?.txt')).toBe('filename.txt');
        expect(ValidationUtils.sanitizeFileName('file"name*.txt')).toBe('filename.txt');
      });
    ...
    describe('validateFilePath', () => {
      it('should validate file paths within intended directory', () => {
        expect(() => ValidationUtils.validateFilePath('/safe/path/file.txt', '/safe/path')).not.toThrow();
      });

      it('should reject file paths outside intended directory', () => {
        expect(() => ValidationUtils.validateFilePath('/unsafe/path/file.txt', '/safe/path')).toThrow(ScrapingError);
      });
    });
  });
  ...
  describe('ScrapingError', () => {
    ...
    it('should create error with message and additional details', () => {
      const error = new ScrapingError('Test error', 'TEST_CODE', 400, 'http://example.com');
      expect(error.message).toContain('Test error');
      expect(error.code).toBe('TEST_CODE');
  ...
  describe('webScraperTool', () => {
    ...
    it('should handle scraping errors', async () => {
      await expect(webScraperTool.execute({
        context: { url: 'https://example.com' },
        runtimeContext: new RuntimeContext(),
        tracingContext: createMockTracingContext()
  ...
  describe('htmlToMarkdownTool', () => {
    ...
    it('should handle conversion errors', async () => {
      ...
      await expect(htmlToMarkdownTool.execute({
  ...
  describe('batchWebScraperTool', () => {
    it('should scrape multiple URLs', async () => {
      const result = await batchWebScraperTool.execute({
        context: { urls: ['https://example.com', 'https://example.org'] },
        runtimeContext: new RuntimeContext(),
        tracingContext: createMockTracingContext()
      });
