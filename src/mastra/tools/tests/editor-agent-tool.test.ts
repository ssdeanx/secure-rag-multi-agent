...
describe('Editor Agent Tool', () => {
    ...
    describe('successful content editing', () => {
        it('should edit blog content successfully with minimal parameters', async () => {
            const mockResponse = {
                editedContent: 'This is an improved blog post with better structure and clarity.',
                contentType: 'blog',
                changes: ['Improved sentence structure', 'Enhanced clarity'],
                suggestions: ['Consider adding more examples']
            }
            mockAgent.generate.mockResolvedValue({ text: JSON.stringify(mockResponse) })
            ...
            expect(mockAgent.generate).toHaveBeenCalledWith('Edit the following content:\n\nThis is a blog post that needs editing.')
        })

        it('should edit technical content with all parameters', async () => {
            const mockResponse = {
                editedContent: 'This technical documentation has been improved with clearer explanations.',
            ...
            expect(mockAgent.generate).toHaveBeenCalledWith('Edit the following content (content type: technical) with a technical tone. Additional instructions: Focus on clarity and add examples:\n\nTechnical content to edit')
        })

        it('should edit business content', async () => {
            const mockResponse = {
                editedContent: 'This business proposal has been professionally edited.',
        ...
        it('should handle non-JSON response with fallback parsing', async () => {
            const plainTextResponse = 'This is the edited content without JSON structure.'
            ...
            expect(result).toEqual({
                editedContent: 'This is the edited content without JSON structure.',
                contentType: 'general',
        ...
        it('should handle JSON response with missing fields', async () => {
            ...
            mockAgent.generate.mockResolvedValue({ text: JSON.stringify(incompleteResponse) })
        ...
        it('should handle JSON response with alternative field names', async () => {
            ...
            mockAgent.generate.mockResolvedValue({ text: JSON.stringify(alternativeResponse) })
            ...
            expect(result.editedContent).toBe('Alternative field name for content')
    ...
    describe('error handling', () => {
        it('should handle agent generation errors', async () => {
            ...
            await expect(editorTool.execute({
