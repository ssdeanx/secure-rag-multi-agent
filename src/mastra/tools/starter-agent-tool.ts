...
export const starterAgentTool = createTool({
  id: "starter-agent",
  description: "Dynamically invokes any available agent based on context and requirements. This tool can call research agents, content creation agents, editing agents, or any other agent in the system.",
  inputSchema: z.object({
    agentType: z.enum(["research", "copywriter", "editor", "assistant", "productRoadmap"]).describe("The type of agent to invoke"),
    task: z.string().describe("The specific task or request for the agent"),
    context: z.string().optional().describe("Additional context or background information"),
    parameters: z.any().optional().describe("Additional parameters specific to the agent type"),
    expectedOutput: z.string().optional().describe("Description of expected output format or structure")
  }),
  outputSchema: z.object({
    agentCalled: z.string().describe("The name of the agent that was invoked"),
    result: z.string().describe("The result from the agent"),
    success: z.boolean().describe("Whether the agent call was successful"),
    metadata: z.any().optional().describe("Additional metadata about the agent call")
  }),
  execute: async ({ context, mastra, tracingContext }) => {
    const { agentType, task, context: additionalContext, parameters = {}, expectedOutput } = context;
    ...
    try {
      ...
      log.info(`Invoking agent: ${agentId} for task: ${task.substring(0, 100)}...`);
      ...
      log.error('Starter agent tool error:', { error: errorMsg, agentType, task: task.substring(0, 100) });
