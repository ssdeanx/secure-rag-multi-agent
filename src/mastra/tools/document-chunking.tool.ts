...
const CustomDocumentChunkingInputSchema = z.object({
  ...
  chunkingStrategy: z.enum(["recursive", "character", "token", "markdown", "semantic-markdown", "html", "json", "latex", "sentence"]).default("recursive"),
...
Use this tool when you need advanced document processing with metadata extraction.
  ...
  execute: async ({ context, tracingContext }) => {
    ...
    try {
      ...
      const buildChunkParams = (strategy: string, maxSize: number, overlap: number, extract: ExtractParams) => {
        const baseParams = {
          maxSize,
          overlap,
          extract,
...
content indexing, or semantic search capabilities.
  ...
  execute: async ({ context, tracingContext }) => {
    ...
    try {
      ...
      const buildChunkParams = (strategy: string, maxSize: number, overlap: number) => {
        const baseParams = {
          maxSize,
          overlap,
        };
      ...
      const chunkingStartTime = Date.now();
      const chunkParams = buildChunkParams(context.chunkingStrategy, context.chunkSize, context.chunkOverlap);
      const chunks = await document.chunk(chunkParams);
      const chunkingTime = Date.now() - chunkingStartTime;
