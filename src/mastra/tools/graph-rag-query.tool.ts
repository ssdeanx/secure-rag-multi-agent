...
export const graphRagQueryTool = createTool({
    id: 'graph-rag-query',
    description:
        'Graph-based RAG query with relationship traversal and enhanced context discovery using PgVector',
    ...
    execute: async ({ context, runtimeContext, mastra, tracingContext }) => {
        ...
        try {
            ...
            log.info(
                `[${requestId}] 🕸️ GRAPH RAG QUERY TOOL CALL #${currentCount}`,
                {
                    context,
                    includeRelated: context.includeRelated,
                    maxHops: context.maxHops,
            ...
            if (context.includeRelated && baseResults.length > 0) {
                log.info(
                    `[${requestId}] 🔗 Enriching ${baseResults.length} results with graph relationships`
                )
            ...
            log.info(
                `[${requestId}] ✅ GRAPH RAG QUERY TOOL CALL #${currentCount} COMPLETED - Found ${enrichedResults.length} results`
            )
            ...
            setTimeout(() => {
                ...
                log.debug(
                    `[${requestId}] 📊 FINAL GRAPH TOOL CALL COUNT: ${finalCount}`
                )
