...
export class Project {
    ...
    get videoId() {
        ...
        for (const pattern of patterns) {
            ...
            if (match?.[1]) {
                log.info(
                    `[Project] Successfully extracted videoId: ${match[1]}`
                )
                return match[1]
            }
        }
    ...
    async getStats(): Promise<{
        ...
        const agentsCount = files.filter(
            (f) =>
                /[/\\]agents[/\\]/.test(f) ||
                /new\s+Agent\s*\(/.test(readText(f))
        ).length
        const toolsCount = files.filter(
            (f) =>
                /[/\\]tools[/\\]/.test(f) || /createTool\s*\(/.test(readText(f))
        ).length
        const workflowsCount = files.filter(
            (f) =>
                /[/\\]workflows[/\\]/.test(f) ||
                /createWorkflow\s*\(/.test(readText(f))
        ).length
        ...
        const detectedTechnologies: Record<string, boolean> = {
            ...
            arcade:
                hasDep('@arcadeai/arcadejs') || anyFileIncludes(/arcade-ai/),
            ...
            confidentAi:
                hasDep('@mastra/evals') ||
                anyFileIncludes(/\bevals?\b|Metric\s*\{/),
            rag:
                hasDep('chromadb') ||
                anyFileIncludes(/retrieval|vector|embedding/i),
            auth:
                hasDep('@workos/node') || anyFileIncludes(/auth|oauth|login/i),
            webBrowsing:
                hasDep('browserbase') ||
    ...
    private safeReadJSON(file: string): any | undefined {
        ...
        try {
            ...
            log.warn(`[Project] Failed to read/parse JSON file ${file}:`, {
    ...
    private async conductNpmInstall(folder: string): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            childProcess.on('close', (code) => {
                log.info(
                    `[Project] npm install completed with exit code: ${code}`
                )
                ...
                if (code === 0) {
                    ...
                    log.error(
                        `[Project] npm install failed with exit code: ${code}`
                    )
                    reject(
