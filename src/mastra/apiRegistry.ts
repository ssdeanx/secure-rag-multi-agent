...
import { createSSEStream, streamProgressUpdate, streamJSONEvent } from '../utils/streamUtils';
  ...
  registerApiRoute('/content/generate', {
    ...
    handler: async (c) => {
      try {
        ...
        if (result.status === 'success') {
          log.info('Content generation completed', { contentType: inputData.contentType });
          return c.json(result.result);
        }

        throw new Error(`Workflow failed with status: ${result.status}`);
      } catch (error) {
        log.error('Content generation API error', { error: error instanceof Error ? error.message : String(error) });
        return c.json({ error: error instanceof Error ? error.message : 'Internal error' }, 500);
      }
    },
  }),
  ...
  registerApiRoute('/content/generate/stream', {
    ...
    handler: async (c) => {
      try {
        ...
        log.error('Content generation stream API error', { error: error instanceof Error ? error.message : String(error) });
        return c.json({ error: error instanceof Error ? error.message : 'Internal error' }, 500);
      }
    },
  }),
