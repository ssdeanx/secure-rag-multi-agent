...
globalThis.roadmapRelationshipAnalysisWorkflow = { execute: async (_: any) => ({ relationships: [], insights: [] }) } as unknown as typeof globalThis & { execute: (args: unknown) => Promise<unknown> };
// eslint-disable-next-line @typescript-eslint/no-explicit-any
globalThis.RoadmapStreamInput = { safeParse: (val: any) => ({ success: true, data: { ...val } }) } as unknown;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
globalThis.RoadmapSyncInput = { safeParse: (val: any) => ({ success: true, data: { ...val } }) } as unknown;
