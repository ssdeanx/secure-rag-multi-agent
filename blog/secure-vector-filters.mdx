...
Vector search without _pre-query_ security constraints is an exfiltration bug waiting to ship. In this post we outline a pattern for binding **who** and **what** to **which** vectors are even _eligible_ for semantic expansion.
...
Security belongs _inside_ the retrieval operator, not bolted on after.
