...
> Enterprises want the creativity and acceleration of AI—but *must not* leak sensitive knowledge or weaken compliance posture.
...
3. **Security-Enforced Retrieval** — Access filters *applied at vector search time* (not just post filtering).
...
const results = await vectorStore.query({
  embedding,
  filter: { classification: { $in: allowed.classifications }, dept: { $in: allowed.departments } },
  topK: 12,
});
```

Enforce *before* model context expansion—never after.
...
Secure AI augmentation is an *architecture problem*, not just a prompt problem. This project gives you the scaffolding to build responsibly from day zero.
